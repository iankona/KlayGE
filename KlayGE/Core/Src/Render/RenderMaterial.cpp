	...
	public:
		...
		void SubThreadStage()
		{
			...
			if (albedo_node)
			{
				...
				if (attr)
				{
					mtl_desc_.mtl_data->albedo_tex_name = attr->ValueString();
				}
			}
			...
			if (metalness_node)
			{
				...
				if (attr)
				{
					mtl_desc_.mtl_data->metalness_tex_name = attr->ValueString();
				}
			}
			...
			if (glossiness_node)
			{
				...
				if (attr)
				{
					mtl_desc_.mtl_data->glossiness_tex_name = attr->ValueString();
				}
			}
			...
			if (emissive_node)
			{
				...
				if (attr)
				{
					mtl_desc_.mtl_data->emissive_tex_name = attr->ValueString();
				}
			}
			...
			if (normal_node)
			{
				...
				if (attr)
				{
					mtl_desc_.mtl_data->normal_tex_name = attr->ValueString();
				}
			}
			...
			if (height_node)
			{
				...
				if (attr)
				{
					mtl_desc_.mtl_data->height_tex_name = attr->ValueString();
				}
		...
		std::shared_ptr<void> MainThreadStage()
		{
			...
			if (!*mtl_desc_.mtl)
			{
				...
				mtl->albedo_tex_name = mtl_desc_.mtl_data->albedo_tex_name;
				mtl->metalness_tex_name = mtl_desc_.mtl_data->metalness_tex_name;
				mtl->glossiness_tex_name = mtl_desc_.mtl_data->glossiness_tex_name;
				mtl->emissive_tex_name = mtl_desc_.mtl_data->emissive_tex_name;
				mtl->normal_tex_name = mtl_desc_.mtl_data->normal_tex_name;
				mtl->height_tex_name = mtl_desc_.mtl_data->height_tex_name;
...
namespace KlayGE
{
	...
	void SaveRenderMaterial(RenderMaterialPtr const & mtl, std::string const & mtlml_name)
	{
...

		{
			...
			{
				albedo_node->AppendAttrib(doc.AllocAttribString("texture", mtl->albedo_tex_name));
			}

			root->AppendNode(albedo_node);
		}

		if ((mtl->metalness > 0) || (!mtl->metalness_tex_name.empty()))
		{
			...
			{
				metalness_node->AppendAttrib(doc.AllocAttribString("texture", mtl->metalness_tex_name));
			}

			root->AppendNode(metalness_node);
		}

		if ((mtl->glossiness > 0) || (!mtl->glossiness_tex_name.empty()))
		{
			...
			{
				glossiness_node->AppendAttrib(doc.AllocAttribString("texture", mtl->glossiness_tex_name));
			}

			root->AppendNode(glossiness_node);
		}

		if ((mtl->emissive.x() > 0) || (mtl->emissive.y() > 0) || (mtl->emissive.z() > 0) || (!mtl->emissive_tex_name.empty()))
		{
			...
			{
				emissive_node->AppendAttrib(doc.AllocAttribString("texture", mtl->emissive_tex_name));
			}
		...
		if (!mtl->normal_tex_name.empty())
		{
			...
			normal_node->AppendAttrib(doc.AllocAttribString("texture", mtl->normal_tex_name));
		...
		if (!mtl->height_tex_name.empty())
		{
			...
			height_node->AppendAttrib(doc.AllocAttribString("texture", mtl->height_tex_name));
